export const TRANSCRIPTION_PROMPT = `Transcribe this single structured math homework worksheet page. Read every visible printed math problem and the student's handwritten answer in reading order.
...
export function verificationPrompt(firstPass: unknown) {
  return `Act as a strict, independent handwriting verifier for this worksheet image. The first-pass transcription JSON is supplied below. Re-inspect the original pixels for every problem in the same array order.
